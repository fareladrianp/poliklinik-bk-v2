        ...
        <div class="flex-1 flex flex-col ml-64">
            ...
            <main class="flex-1 container mx-auto px-6 py-6">
                <div class="bg-white rounded shadow p-6">
                    <h3 class="text-2xl font-bold mb-4">Kelola Data Poli</h3>
                    <p class="text-xl mb-4">Tambah Data Poli</p>
                    <form action="{{ route('tambah.poli') }}" method="post" class="mb-4">
                        @csrf
                        <div class="grid gap-6 mb-6 md:grid-cols-2">
                            <div>
                                <label for="first_name" class="block mb-2 text-sm font-medium text-black">Nama Poli</label>
                                <input type="text" name="nama_poli" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                            </div>
                            <div>
                                <label for="last_name" class="block mb-2 text-sm font-medium text-black">Keterangan</label>
                    ...
                    <p class="text-xl mb-4">Daftar Poli</p>
                    <table class="border border-slate-500">
                        <thead>
                          <tr>
                            <th class="border border-slate-600 py-2 px-2">No.</th>
                            <th class="border border-slate-600 py-2 px-24">NamaPoli</th>
                        ...
                        <tr>
                            ...
                            <td class="border border-slate-600 py-1 text-center">{{ $data->nama_poli }}</td>
                            <td class="border border-slate-600 py-1 text-center">{{ $data->keterangan}}</td>
                            <td class="border border-slate-600 py-1 text-center">
                               <div class="flex justify-center">
                                ...
                                <div id="edit-modal-{{ $data->id }}" tabindex="-1" aria-hidden="true" 
                                    ...
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="bg-white rounded-lg shadow dark:bg-gray-400">
                                            ...
                                            <!-- Modal Body -->
                                            <form action="{{ route('update.poli', $data->id) }}" method="post" class="p-4" >
                                                @csrf
                                                <div class="mb-4">
                                                    <label class="block text-sm font-medium text-gray-700">Nama Poli</label>
                                                    <input type="text" name="nama_poli" value="{{ $data->nama_poli }}" 
                                ...
                                <form action="{{ route('delete.poli',  $data->id ) }}" method="post">
                                    @csrf
