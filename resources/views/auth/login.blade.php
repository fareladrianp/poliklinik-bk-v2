...
<div class="bg-slate-200 pt-0.5 pb-60">
    <div class="flex justify-center mt-36">
        <div class="w-full max-w-xs ">
                <form action="{{ route('login.submit') }}" class="bg-white shadow-2xl rounded-xl px-8 pt-6 pb-8 mb-4" method="post">
                    @csrf
                    <h1 class="flex justify-center font-bold text-3xl mb-5">LogIn</h1>
                  <div class="mb-4">
                  ...
                  @if (session('error'))
                      <div style="color: red;" class="flex justify-center pb-4">
                          {{ session('error') }}
                      </div>
                    @endif
